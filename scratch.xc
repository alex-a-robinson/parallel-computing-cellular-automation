...
interface worker_farmer {
    ...
    [[guarded]] void tick(unsigned int* strip_ref[], uint start_index, uint stop_index, uint width);
};
...
void worker(int id, server interface worker_farmer wf_i) {
    ...
    // while (1) {
    //     select {
    //         case wf_i.tick(unsigned int* strip_ref[], uint start_index, uint stop_index, uint width):
    //             // TODO: work
